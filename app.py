...
.divider { border: none; border-top: 0.5px solid #1f1f1f; margin: 1.5rem 0; }
.lambda-display {
    font-family: 'DM Mono', monospace;
    font-size: 0.75rem;
    color: #555;
...
def build_model(data: pd.DataFrame, xi: float = 0.0065, form_window: int = 10, hist_w: float = 0.4):
    df = data.copy()
    ref = df['Date'].max()
    df['w'] = np.exp(-xi * (ref - df['Date']).dt.days)
    form_w = 1.0 - hist_w

    # Full time-weighted historical averages
    def wavg(gc, tc):
    ...
    # Recent form window — last N matches per team only
    def get_recent(team_col, goals_col):
        ...
        for team, group in df.groupby(team_col):
            ...
            rows.append({
                ...
                # confidence factor: penalise if team has fewer matches than window
...
def get_form(data, team, n=6):
    ...
    combined = pd.concat([h[['Date','result']], a[['Date','result']]])
    return combined.sort_values('Date').tail(n)['result'].tolist()
...
with st.sidebar:
    ...
    xi = st.slider(
        "Time decay (xi)", 0.001, 0.015, 0.0065, 0.001,
        help="Higher = old matches fade faster. 0.0065 is the DC paper default."
    )
    form_window = st.slider(
        "Form window (matches)", 3, 20, 10, 1,
        help="How many recent matches count as 'current form'. Lower = more reactive."
    )
    hist_w = st.slider(
        "History vs form weight", 0.0, 1.0, 0.4, 0.05,
        help="0.0 = pure recent form only. 1.0 = pure historical average. 0.4 means 40% history, 60% recent form."
    )
    rho = st.slider(
        "Rho (DC low-score fix)", -0.2, 0.0, -0.1, 0.01,
        help="Corrects underestimation of 0-0 and 1-1. Keep near -0.1."
    )

    st.markdown("---")
    st.markdown(f"""
    <div style="font-family:'DM Mono',monospace;font-size:0.65rem;color:#444;line-height:2.2;">
...
# Build model with sidebar settings
...
with col_right:
    if home == away:
        ...
        # Form bars
        form_h = get_form(data, home)
        form_a = get_form(data, away)

        def form_html(results, label):
            bars = "".join(
                f'<span class="form-bar form-{r.lower()}">{r}</span>'
                for r in results
            )
            return (
                f'<div class="form-wrap">'
                f'<span style="font-family:\'DM Mono\',monospace;font-size:0.65rem;'
                f'color:#444;margin-right:8px;">{label[:3].upper()}</span>{bars}</div>'
            )

        st.markdown(f'<div class="section-heading">{home} vs {away}</div>', unsafe_allow_html=True)
        st.markdown(form_html(form_h, home) + form_html(form_a, away), unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        for col, label, prob, edge, impl in [
            ...
            with col:
                ...
                <div class="metric-card">
                    ...
                    <div class="lambda-display" style="color:{edge_col(edge)}">
                        edge {edge*100:+.1f}%</div>
        ...
        # Value banner
        if best_edge > 0.03:
            label_map = {edge_h: 'HOME WIN', edge_d: 'DRAW', edge_a: 'AWAY WIN'}
            st.markdown(f"""
            <div class="edge-banner">
                ⚑ VALUE DETECTED · {label_map[best_edge]} · +{best_edge*100:.1f}% EDGE
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown("""
            <div class="no-value-banner">NO SIGNIFICANT VALUE DETECTED</div>
            """, unsafe_allow_html=True)

        # Projections + top scorelines
        st.markdown(f"""
        <div class="lambda-display" style="margin-bottom:0.25rem;">
        ...
        sns.heatmap(
            ...
            ax=ax,
            cbar=False,
            xticklabels=list(range(6)),
            yticklabels=list(range(6)),
        )
